...
- Where motion improves continuity, hierarchy, feedback, or spatial understanding, use polished and subtle animation that stays consistent with the current design system. Keep interactions immediate: do not add artificial waiting, animate compositor-friendly `transform` and `opacity` where possible, avoid expensive layout animation on large or virtualized views, and honor the user's reduced-motion preference. Static presentation is preferable when animation would be decorative noise or make the interface feel slower.
