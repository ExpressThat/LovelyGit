...
- The visual-test helper records the exact process it launches. Reuse its healthy tracked instance by default, pass `-Restart` when a clean relaunch is required, and stop it with `scripts/Stop-LovelyGitVisualTest.ps1`. Never broadly kill LovelyGit, `dotnet`, or WebView2 processes; an untracked instance must be handled explicitly by the user.
...
- Keep `PERFORMANCE.md` current in the same commit as every performance change. Record the affected workflow, before/after evidence, fixture, memory or process trade-offs, commit reference, and any rejected experiment worth preventing future agents from repeating.
...
- Keep the complete backend gate below one minute on the development machine; the established baseline is roughly 30-36 seconds for 572 tests. Treat a sustained regression above one minute as a test-performance bug and profile it before adding more integration work.
- Before an expensive test or visual-test journey, run `powershell -NoProfile -ExecutionPolicy Bypass -File scripts/Test-LovelyGitSessionHealth.ps1`. Resolve reported leftover test trees or untracked LovelyGit instances before starting more work; never stack another suite on top of an unknown runner.
- Run the backend gate through `scripts/Invoke-LovelyGitTestGate.ps1` instead of invoking a long-running `dotnet test` directly. Its Windows Job Object terminates the complete owned process tree on timeout, cancellation, wrapper failure, or interrupted agent work. Adjust its explicit timeout when justified; do not bypass it because a previous run was slow.
- Validate changes to process ownership, test cleanup, or visual launching with `scripts/Test-ProcessSafety.ps1`. Safety tests must prove forged or stale ownership records cannot terminate unrelated processes.
- Do not launch `git.exe` merely to test validation, argument construction, cancellation before execution, simulated dependency failure, or LovelyGit-owned rollback logic. Validate before repository discovery where safe, inject the narrow command-runner boundary for simulated outcomes, and still assert the surfaced error plus concrete no-mutation state. Retain representative real-Git tests wherever Git's own semantics are the contract.
- Seed repeated Git scenarios from `RepositoryTemplate<TState>` and `InitializedRepositoryTemplate`, then give each test an exclusive disposable copy. Never share a mutable worktree between tests, and do not repeatedly run identical `git init`, identity configuration, or base-commit commands in every test.
- Keep reusable Git seeds inside `RepositoryTemplate<TState>` rather than a custom static temporary directory. The template lifetime groups seeds under one owned testhost root and the official gate reclaims it after exit; bypassing it leaks thousands of small Git objects into `%TEMP%` across repeated runs.
- Do not launch Git solely to assert state that the test can read authoritatively from HEAD, refs, or config after the production command has completed. Keep real Git for the mutation and for semantics only Git defines; use direct assertion-side reads to avoid process contention without weakening the observable outcome.
- For a performance contract that must not scale with unrelated refs, prefer `PackedRefFixture`: it keeps a representative loose-ref sample and stores the bulk in `packed-refs`, so Git still sees the complete ref set without thousands of fixture-only filesystem operations. Use thousands of loose files only when loose-ref enumeration itself is the behavior under test.
- Keep nested-shell integrations that suffer severe process contention in `GitShellIntegrationCollection`. Add a class only when isolated-versus-full timing proves substantial inflation; do not serialize ordinary Git/parser tests or use the collection to hide a slow implementation.
- Keep nested-shell classes sequential with each other, but do not globally disable unrelated test parallelism unless repeated isolated-versus-full evidence proves it necessary. Allocation and latency gates remain in the globally isolated `PerformanceTestCollection` because their counters are process-wide.
- Measure test timing from a clean runner state. Before trusting a comparison, ensure no orphaned `testhost` or prior `dotnet test` process remains after a timeout or interrupted command, and confirm improvements with two complete `dotnet test LovelyGit.slnx` runs.
- Do not reduce runtime by deleting assertions, collapsing materially different outcomes, skipping tests, weakening repository-state checks, or replacing all real-Git coverage with mocks. Performance work must preserve or strengthen the observable safety contract.
