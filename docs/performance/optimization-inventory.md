...
- Virtualized the top repository strip and New Tab's recent-repository list above 30 records, while preserving the existing Motion tab layout for ordinary collections, exact filtering, lazy row overlays, active-tab visibility, and both complete custom-scrollbar ranges (current known-repository-surface checkpoint).
