...
- Reconciled successful branch pushes directly instead of waiting for the repository watcher. The remote ref now appears 62 ms after completion instead of 278 ms, while the existing branch-action path already paints busy feedback in 18-21 ms without an extra forced delay (current branch-push checkpoint).
