...
- Rendered ordinary commit changed-file lists of up to 10 visible rows without allocating a virtualizer or resize observer, added an immediate bootstrap for maximum lists, and reduced large-list overscan from six rows to four while preserving the complete scroll and context-menu surfaces (current changed-file-list checkpoint).
