...
- Corrected Fetch, Pull, and Push from one-way native contracts to completion responses, so their promises, success messages, refreshes, and busy controls now follow the real Git process instead of only message dispatch (current remote-transport checkpoint).
- Shared the proven two-frame browser paint boundary with remote synchronization. In a delayed disposable local transport, Fetch, Pull, and Push painted disabled in 1-5 ms and remained protected for 3.37-3.50 seconds until their upload/receive process exited; exact refs, `HEAD`, and remote state matched afterward with clean page/error-console gates (current remote-transport checkpoint).
