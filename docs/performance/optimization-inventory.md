...
- Replaced the ordinary 2-30 repository strip's browser-default horizontal scrollbar with a two-pixel semantic scrollbar, preventing the native gutter from displacing and clipping the tab buttons while preserving user scrolling and access to every tab (current 20-repository switch checkpoint).
