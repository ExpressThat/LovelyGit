...
- Read tiny worktree `HEAD`, `gitdir`, and lock files through stack buffers, then raised the measured bounded fan-out from eight to 32 reads for large linked-worktree collections (`48a56ad`, pending commit).
...
- Parallelized loose local/remote ref reads, raised the measured bounded path from eight to 32 workers for large loose-ref collections, stopped parsing object IDs merely to fingerprint loose refs, and retained exact malformed-ref, tag-limit, cancellation, and cache semantics (`23e20e0`, pending commit).
