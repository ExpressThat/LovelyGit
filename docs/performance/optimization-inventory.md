...
- Verified a real 5,000-file local submodule cycle through CMG: Initialize paints its disabled busy state in 1.1 ms and reaches authoritative `Current` state in 3.64 s including Git and the native refresh. The equivalent direct Git initialization took 6.20 s in the same disposable fixture, so the existing asynchronous command path was retained without an unnecessary paint delay (current submodule-operation audit).
