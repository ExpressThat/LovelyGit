...
internal sealed class RenameBranchCommandResolver
    : CommandResponder<RenameBranchCommandArguments>
{
    ...
    {
        return command.CommandType == NativeMessageType.RenameBranch;
    }
        ...
        NativeCommand<RenameBranchCommandArguments> command)
    {
        ...
        if (!GitBranchNameValidator.IsValidBranchName(arguments.OldBranchName)
            || !GitBranchNameValidator.IsValidBranchName(arguments.NewBranchName))
        {
        ...
        try
        {
            await _branchCommandService.RenameBranchAsync(
                foundRepo.Path,
                arguments.OldBranchName.Trim(),
                arguments.NewBranchName.Trim(),
    ...
    private static CommandResponseBase Success(NativeCommand<RenameBranchCommandArguments> command)
    {
