...
internal sealed class SetBranchUpstreamCommandResolver
    : CommandResponder<SetBranchUpstreamCommandArguments>
{
    ...
    protected override JsonTypeInfo<SetBranchUpstreamCommandArguments> ArgumentsJsonTypeInfo =>
        BranchesJsonSerializerContext.Default.SetBranchUpstreamCommandArguments;

    public SetBranchUpstreamCommandResolver(
    ...
    {
        return command.CommandType == NativeMessageType.SetBranchUpstream;
    }

    public override async Task<CommandResponseBase> Resolve(
        NativeCommand<SetBranchUpstreamCommandArguments> command)
    {
        ...
        if (!GitBranchNameValidator.IsValidBranchName(arguments.BranchName)
            || !GitBranchNameValidator.IsValidBranchName(arguments.UpstreamName))
        {
            return Failure(command, "Branch upstream is not valid.");
        }
        ...
        try
        {
            await _branchCommandService.SetBranchUpstreamAsync(
                foundRepo.Path,
                arguments.BranchName.Trim(),
                arguments.UpstreamName.Trim(),
    ...
    private static CommandResponseBase Success(NativeCommand<SetBranchUpstreamCommandArguments> command)
    {
